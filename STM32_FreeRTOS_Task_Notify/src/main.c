...
int main(void)
{
	...
	xTaskCreate(led_task_handler,"TASK-LED",500,NULL,2,&xTaskHandle1);

	//lets create button_task
	xTaskCreate(button_task_handler,"TASK-BUTTON",500,NULL,2,&xTaskHandle2);
...
{
	while(1)
	{//Esperara hasta recibir una notificacion de button_task
		if(xTaskNotifyWait(0,0,NULL,portMAX_DELAY) ==pdTRUE)
		{
			//we received the notification. so lets toggle the LED
			GPIO_ToggleBits(GPIOA,GPIO_Pin_5);
			sprintf(usr_msg,"Notificado vamo a chambiar\r\n");
			printmsg(usr_msg);
		}
	}
}
